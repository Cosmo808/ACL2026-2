...
from utils.xnli.arguments import ModelArguments, DataTrainingArguments, CustomTrainingArguments, AdapterArguments
from utils.xnli.dataloader import load_raw_datasets
from utils.xnli.model import prepare_model_and_tokenizer_and_collator_and_encoder
from utils.xnli.trainer import prepare_trainer
...
if __name__ == "__main__":
    ...
    out_prefix = "E:/ACL2026-2/output_lora_peft/dynamic_tk"
    method = "lora"
    dropout = 0.3
    rank = 128
    seed = 42
    tokenization_type = "dynamic"
    dynamic_tokenization_merges = 20
    exp_type = "dynamic_bpe"
    lng = "en"
    alpha = 2 * rank

    today = datetime.date.today().strftime("%d%m%Y")
    output_dir = f"{out_prefix}/{exp_type}_{lng}_{method}_alpha_{alpha}_dropout_{dropout}_rank_{rank}_seed_{seed}_merges_{dynamic_tokenization_merges}_{today}"

    model_args = ModelArguments(
        model_name_or_path=model_name_or_path,
        tokenization_type=tokenization_type,
        dynamic_tokenization_merges=dynamic_tokenization_merges,
    )
    ...
    # Prepare the model
    ###########################################################################
    model, hypernet, tokenizer, compute_metrics, datasetEncoder = prepare_model_and_tokenizer_and_collator_and_encoder(
    ...
    trainer = prepare_trainer(
        model, training_args, raw_datasets, tokenizer, compute_metrics, model_args, datasetEncoder, eval_dataset
    )
