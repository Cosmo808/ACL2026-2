...
def convert_to_byte_level(
    ...
    temp_file_obj = NamedTemporaryFile(mode='w', delete=False, encoding='utf-8')
    temp_file_name = temp_file_obj.name
    temp_file_obj.close()
    if match_special_tokens_to is not None:
        match_special_tokens_to._tokenizer.save(temp_file_name)
        with open(temp_file_name, 'r', encoding='utf-8') as f_read:
            match_special_tokens_to_data = json.load(f_read)
    else:
        match_special_tokens_to_data = {}
    tokenizer._tokenizer.save(temp_file_name)
    with open(temp_file_name, 'r', encoding='utf-8') as f_read:
        tokenizer_data = json.load(f_read)
    ...
    with open(temp_file_name, "w", encoding='utf-8') as f_write:
        json.dump(tokenizer_data, f_write)
    tokenizer._tokenizer = Tokenizer.from_file(temp_file_name)
